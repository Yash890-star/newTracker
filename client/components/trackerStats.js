...
const TrackerStats = (props) => {
    ...
    if (props.data) {
        ...
        let b = <div className={`rounded-2xl py-10 flex flex-col  ${ss.thBackground}  basis-1/3 ml-3 flex-none`}>
            ...
            <h1 className="text-center mb-2 text-xl">Total Problems Remaining: {" " + props.data.solved-props.data.assigned}</h1>
            <div className={`${s.circularProgressbar}`}>
            <div style={{ width: 200, height: 200 }} className="mx-auto mt-4">
                <CircularProgressbar value={Math.round(props.data.assigned / props.data.solved * 100)} text={`${Math.round(props.data.assigned / props.data.solved * 100)}%`}
                    styles={buildStyles({
                        pathColor: `#2563eb`,
                        textColor: '#2563eb',
                        trailColor: '#d6d6d6',
